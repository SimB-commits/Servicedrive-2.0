...
const formatValue = (value: any, fieldType: string): string => {
  ...
  // Explicit hantering baserat på fälttyp
  switch (fieldType) {
    case "DATE":
    case "DUE_DATE":
      // Hantera datum specifikt
      ...
      // Striktare datumvalidering
      if (typeof value === "string") {
        // Använd en regex för att säkerställa datumformatet
    ...
    case "NUMBER":
      // För nummer, konvertera till sträng men tillåt bara numeriska värden
      return typeof value === "number" ? value.toString() : 
             typeof value === "string" && /^-?\d+(\.\d+)?$/.test(value) ? value : "";
    
    case "TEXT":
    default:
      // För text, returnera som sträng
      ...
      const prepareDynamicFields = () => {
        ...
        selectedTicketType.fields.forEach((field: Field) => {
          ...
          if (field.fieldType === "DUE_DATE") {
            dueDateValue = formatValue(value, field.fieldType);
          } else {
            // Skicka med fälttypen för korrekt formatering
