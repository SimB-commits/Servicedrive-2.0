...
import { triggerSubscriptionUpdate } from '@/utils/subscriptionEvents';
...
const PlanSelector: React.FC<PlanSelectorProps> = ({
  ...
  // Använd vår globala prenumerationskontext istället för props
  const subscription = useSubscription();
  
  const [selectedPlan, setSelectedPlan] = useState<SubscriptionPlan>(subscription.plan || 'STARTUP');
  const [billingPeriod, setBillingPeriod] = useState<'monthly' | 'yearly'>(subscription.billingPeriod || 'monthly');
  const [autoRenew, setAutoRenew] = useState<boolean>(subscription.autoRenew || false);
  ...
  const calculatePriceDifference = () => {
    ...
    const currentPrice = subscription.billingPeriod === 'yearly'
      ? SubscriptionService.getYearlyPrice(subscription.plan)
      : SubscriptionService.getMonthlyPrice(subscription.plan);
  ...
  const handleConfirmChange = async () => {
    ...
    const isDowngrade = SubscriptionService.isPlanDowngrade(subscription.plan, selectedPlan);
    let downgradeWarnings: string[] | undefined;
    
    if (isDowngrade) {
      // Hämta planer för jämförelse
      const currentLimits = SubscriptionService.getLimits(subscription.plan);
  ...
  const submitPlanChange = async () => {
    ...
    try {
      ...
      // Utlös en global uppdatering av prenumerationsdata
      ...
      // Återladda sidan efter 2 sekunder för att uppdatera UI
      // OBS: Detta kan nu vara överflödigt eftersom vi har en global uppdatering
      setTimeout(() => {
          ...
          <section className="mt-4">
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                isDisabled={isSubmitting || !!successMessage || subscription.isLoading}
              >
