...
import { checkAndRenewSubscriptions, checkUsageLimits } from './scheduledSubscriptionTasks';
...
/**
 * Huvudfunktion för att köra alla schemalagda uppgifter
 * Denna kan anropas från en CRON-job eller en scheduler
 */
export async function runAllScheduledTasks(): Promise<void> {
  try {
    logger.info('Kör alla schemalagda uppgifter');
    
    // Återställ månadsantal för ärenden om det behövs
    await resetMonthlyTicketCountIfNeeded();
    
    // Arkivera gamla ärenden
    await archiveOldTickets();
    
    // Kontrollera och förnya prenumerationer
    await checkAndRenewSubscriptions();
    
    // Kontrollera användning mot plangränser
    await checkUsageLimits();
    
    logger.info('Alla schemalagda uppgifter slutförda');
  } catch (error) {
    logger.error('Fel vid körning av schemalagda uppgifter', { 
      error: error instanceof Error ? error.message : 'Okänt fel'
    });
  }
}
